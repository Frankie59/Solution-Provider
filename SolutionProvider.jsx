...
const SolutionProvider = () => {
  ...
  const handleCalculateCs = () => {
    const { SDS, SD1, T, Ie, R } = inputs;
    const Cs = calculateCs(parseFloat(SDS), parseFloat(SD1), parseFloat(T), parseFloat(Ie), parseFloat(R));
  ...
  const handleCalculateV = () => {
    ...
    const V = calculateV(parseFloat(Cs), totalWeight);
  ...
  const handleCalculateFvx = () => {
    ...
    const { Cvx, V } = results;
    const Fvx = calculateFvx(weights.map(w => parseFloat(w)), Cvx.map(c => parseFloat(c)), parseFloat(V));
        ...
        {activeTab === 'baseShear' && (
          <div className="grid grid-cols-2 gap-4">    
            ...
            <div className="bg-blue-100 rounded-lg p-4">
              ...
              <h3 className="text-md font-medium mb-4">Formatted Output</h3>
              <div className="border p-4 rounded mb-4">
                <p>Base Shear (V): <span className="font-bold">{results.V}</span> kips</p>
              </div>
              <h3 className="text-md font-medium mb-4">Code Output</h3>
              <pre className="bg-gray-100 p-4 rounded">
                {`V: ${results.V}`}
              ...
              {inputs.weights.map((weight, index) => (
                <div key={index} className="mb-4">
                  ...
                  <input
                    ...
                    className="w-full p-2 border rounded"
                  />
                  <label className="block mb-2 mt-2">Height (ft)</label>
                  <input
                    type="text"
            ...
            <div className="bg-blue-100 rounded-lg p-4">
              ...
              <div className="mb-4">
                <label className="block mb-2">Seismic Coefficient (Cvx)</label>
                <output className="w-full p-2 border rounded block">{results.Cvx.join(', ')}</output>
              </div>
              <h3 className="text-md font-medium mb-4">Formatted Output</h3>
              <div className="border p-4 rounded mb-4">
                <p>Seismic Coefficient (Cvx): <span className="font-bold">{results.Cvx.join(', ')}</span></p>
              </div>
              <h3 className="text-md font-medium mb-4">Code Output</h3>
              <pre className="bg-gray-100 p-4 rounded">
            ...
            <div className="bg-blue-100 rounded-lg p-4">
              ...
              <div className="mb-4">
                <label className="block mb-2">Seismic Force (Fvx)</label>
                <output className="w-full p-2 border rounded block">{results.Fvx.join(', ')}</output>
              </div>
              <h3 className="text-md font-medium mb-4">Formatted Output</h3>
              <div className="border p-4 rounded mb-4">
                <p>Seismic Force (Fvx): <span className="font-bold">{results.Fvx.join(', ')}</span> kips</p>
              </div>
              <h3 className="text-md font-medium mb-4">Code Output</h3>
              <pre className="bg-gray-100 p-4 rounded">
